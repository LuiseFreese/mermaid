...
This guide explains how to use the Mermaid to Dataverse converter effectively, from basic usage to deployment options.
...
3. **Prompt for global choice sets** - Ask if you want to include global choice sets and their location
4. **Show configuration summary** - Review your settings before proceeding
5. **Create the solution** - Build entities and relationships in Dataverse
...
Global choice sets are now defined in separate JSON files, as support for choice fields directly in Mermaid diagrams has been removed.

Create a JSON file (e.g., `global-choices.json`) that defines your global choice sets. Read more in the [Global Choices Guide](/docs/GLOBAL-CHOICES-GUIDE.md).

### Including Global Choice Sets

There are two ways to include global choice sets in your deployment:

1. **Interactive mode**: When running `npm run create`, you'll be asked if you want to include global choice sets and prompted for the file path.

2. **Command line**: Use the `--global-choices` parameter to specify the JSON file:
   ```bash
   npm start convert -- -i my-erd.mmd --solution MySolution --global-choices ./my-choices.json
   ```

### Global Choice Set Format
...
The tool automatically applies your chosen publisher prefix to all global choice sets. You should **not** include the publisher prefix in your JSON file.

For example, with publisher prefix `contoso`, the above choice set would be created as `contoso_PriorityChoices`. 

If you previously had `mint_` or another prefix in your choice set names, the tool will strip those prefixes and apply your chosen publisher prefix instead.
...
- **Use dry runs**: Preview changes before applying to Dataverse
...
- Entity names preserve their original casing from the Mermaid ERD file
