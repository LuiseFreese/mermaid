            ...
            try {
                ...
                if (options.detectCDM !== false) {
                    try {
                        ...
                        result.warnings.push('CDM detection unavailable');
                    }
                }
    ...
    validateEntityStructure(entities, relationships) {
        ...
        entities.forEach((entity, index) => {
            ...
            if (!entity.attributes || !Array.isArray(entity.attributes)) {
                warnings.push(`Entity '${entity.name}' has no attributes defined`);
            } else if (entity.attributes.length === 0) {
                warnings.push(`Entity '${entity.name}' has empty attributes array`);
            }
            ...
            if (!hasPrimaryKey) {
                warnings.push(`Entity '${entity.name}' may be missing a primary key`);
            }
        });
        ...
        if (Array.isArray(relationships)) {
            relationships.forEach((rel, index) => {
                ...
                if (rel.from && !entityNames.includes(rel.from)) {
                    errors.push(`Relationship references unknown entity: ${rel.from}`);
                }
                if (rel.to && !entityNames.includes(rel.to)) {
