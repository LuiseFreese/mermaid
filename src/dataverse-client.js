...
class DataverseClient {
  ...
  async checkSolutionExists(uniqueName) {
    const q1 = `/solutions?$select=solutionid,uniquename,friendlyname&$filter=uniquename eq '${uniqueName}'`;
    ...
    for (const variation of caseVariations) {
      ...
      const q = `/solutions?$select=solutionid,uniquename,friendlyname&$filter=uniquename eq '${variation}'`;
      this._log(' GET', `${this.baseUrl}/api/data/v9.2${q}`);
      try {
  ...
  async createCustomEntities(entities, options = {}) {
    ...
    if (!options.dryRun && Array.isArray(options.relationships) && options.relationships.length) {
    ...
    } else if (options.dryRun && Array.isArray(options.relationships) && options.relationships.length) {
      this._log(` [DRY RUN] Would create ${options.relationships.length} relationships`);
    } else {
      this._log(' No relationships to create');
    }
  ...
  async addGlobalChoicesToSolution(selectedChoices, solutionUniqueName) {
    ...
    for (const choiceName of selectedChoices) {
      try {
        ...
        const matchingChoice = allChoices.value.find(choice => choice.Name === choiceName);
        if (!matchingChoice) {
  ...
  async cleanupTestEntities(options = {}) {
    ...
    this._log(`🧹 Starting cleanup process (dryRun: ${dryRun})`);
    ...
    try {
      ...
      // 4. Delete entities
      if (!dryRun && entitiesToCleanup.length > 0) {
      ...
      results.summary = dryRun 
        ? `Dry run: Would delete ${results.relationshipsFound.length} relationships and ${results.entitiesFound.length} entities`
        : `Deleted ${results.relationshipsDeleted} relationships and ${results.entitiesDeleted} entities`;
        
      this._log(results.summary);
