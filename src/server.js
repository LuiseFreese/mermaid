...
async function initializeComponents() {
  ...
  try {
    ...
    const validationService = new ValidationService({
      ...
      mermaidParser: MermaidERDParser, // Legacy parser for backward compatibility
...
async function routeRequest(pathname, req, res, components, query) {
  ...
  // Wizard UI
  if (req.method === 'GET' && pathname === '/wizard') {
    return components.wizardController.serveWizard(req, res);
  }
  ...
  if (req.method === 'POST' && pathname === '/upload') {
    return components.deploymentController.deploy(req, res);
  }

  if (req.method === 'POST' && pathname === '/cleanup') {
    return components.deploymentController.cleanup(req, res);
  }
